...
class The_Game:
    def __init__(self) -> None:
        ...
        self.score = 0
        self.high_score = 0
        self.player = Player(self.__display_width / 2 - (self.robot_width / 2),
    ...
    def load_items_to_be_dropped(self):
        ...
        for i in range(self.monsters_pcs):
            ...
            self.monsters.append(
                [-1000, self.__display_height, random.choice((-1, 1))])
    ...
    def draw_display(self):
        ...
        score_txt = self.font.render(
            f"Score: {self.score}", True, (244, 96, 54))
        ...
        for i in range(self.monsters_pcs):
            ...
            if self.player.x <= self.monsters[i][0] + (self.monster_width / 2) <= self.player.x + self.robot_width:
                if self.player.y <= self.monsters[i][1] <= self.player.y + self.robot_height or self.player.y <= self.monsters[i][1] + self.monster_height <= self.player.y + self.robot_height:
                    if self.score < 0:
                        game_over_txt = self.font2.render("GAME OVER", True, (244, 96, 54))
                        self.display.blit(
                            game_over_txt, (self.__display_width / 2 - game_over_txt.get_width() / 2, self.__display_height / 2 - game_over_txt.get_height() / 2))
                        pygame.display.flip()
                        self.clock.tick(1)
                        self.clock.tick(1)
                        self.game_over()
        ...
        for i in range(self.coins_pcs):
            ...
            if self.player.x <= self.coins[i][0] + (self.coin_width / 2) <= self.player.x + self.robot_width:
                if self.player.y <= self.coins[i][1] <= self.player.y + self.robot_height or self.player.y <= self.coins[i][1] + self.coin_height <= self.player.y + self.robot_height:
                    self.coins[i][1] = 1500  # Move out of sight
                    self.score += 1
        # Collision with door --> save high score and restart the game
        for i in range(self.doors_pcs):
            self.display.blit(self.images["ovi"],
                              (self.doors[i][0], self.doors[i][1]))
        if self.player.x <= self.doors[i][0] + (self.door_width / 2) <= self.player.x + self.robot_width:
            if self.player.y <= self.doors[i][1] <= self.player.y + self.robot_height or self.player.y <= self.doors[i][1] + self.door_height <= self.player.y + self.robot_height:
                self.game_over()
    ...
    def player_movement(self):
        ...
        # Jump
        if self.player.y != 0:
            if self.key_down_up and self.player.y + self.robot_height == self.__display_height:
                self.player.y -= 120
    ...
    def items_location(self):
        for i in range(self.monsters_pcs):
            ...
            if self.monsters[i][1] > self.__display_height or self.monsters[i][0] > self.__display_width or self.monsters[i][0] < 0:
